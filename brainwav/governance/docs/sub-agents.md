# governance framework Subagents
...
- **Project subagent**: `governance/agents/` (highest priority)
- **User subagent**: `~governance/agents/` (lower priority)
...
| :---------- | :------------------ | :---------------------------- | :------- |
| **Project** | `governance/agents/`   | Available in current project  | Highest  |
| **User**    | `~governance/agents/` | Available across all projects | Lower    |
...
```bash
mkdir -p governance/agents
$EDITOR governance/agents/test-runner.md
```

All subagent files are validated against JSON Schemas in `governance/schemas/agents/*.schema.json`.
...
- **Version control** project subagents in `governance/agents`
